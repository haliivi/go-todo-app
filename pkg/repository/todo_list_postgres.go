...
func (r *TodoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND ul.id = $2", todoListTable, usersListTable)
...
func (r *TodoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id AND ul.list_id = $%d AND ul.user_id = $%d", todoListTable, setQuery, usersListTable, argId, argId+1)
